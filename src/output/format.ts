...
export function formatResult(result: SearchResult, verbose = false): string {
  ...
  if (result.cache) {
    lines.push("## Cache", `- cache_hit: ${result.cache.cacheHit}`, `- chunks: ${result.cache.chunks}; files: ${result.cache.files}; size_mb: ${(result.cache.sizeBytes / (1024 * 1024)).toFixed(2)}`, `- cache_dir: ${result.cache.cacheDir}`, "");
  }
