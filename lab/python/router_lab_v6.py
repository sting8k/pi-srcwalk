#!/usr/bin/env python3
"""
Query Router Lab Test v6

Adds lightweight Model2Vec embedding reranking on top of v4:
- v4 BM25/PRF builds a candidate pool for broad natural-language/test/definition queries
- potion-code-16M reranks that BM25 pool only; it does not build a full vector index
...
def embedding_rerank(plan: v3.QueryPlan, candidates: list[v3.Candidate], pool_size: int, max_results: int) -> tuple[list[v3.Candidate], v3.CommandResult, EmbedMetrics]:
    ...
    query_emb = normalize_rows(model.encode([plan.query]))[0]
    query_encode_ms = int((time.perf_counter() - q_start) * 1000)

    c_start = time.perf_counter()
    cand_embs = normalize_rows(model.encode(texts))
    ...
    for rank, idx in enumerate(order[:max_results], 1):
        ...
        cand.evidence.append(f"potion-code-16M rerank={rank} cosine={sim:.4f}")
    ...
    cmd = v3.SrcwalkCommand(
        "embedding-rerank",
        ["potion-code-16M", "rerank", plan.query, "--pool", str(len(pool))],
        "potion-code-16M rerank over BM25/PRF pool",
        "embedding",
    )
        ...
        if bm25_result.code == 0:
            if should_run_embedding(plan, bm25_candidates, enable_embedding):
                try:
                    ...
                    notes.append(
                        "potion-code-16M reranked BM25 pool; "
...
def format_result(result: v3.SearchResult, verbose: bool = False) -> str:
    ...
    return text.replace("# semantic-search lab v3:", "# semantic-search lab v6:", 1)
...
def run_complex_lab(selected_repo: Optional[str] = None, limit: Optional[int] = None, verbose: bool = False, enable_embedding: bool = True, embed_pool: int = DEFAULT_EMBED_POOL) -> str:
    ...
    chunks.append("# Query Router Lab Test v6 - potion-code-16M Rerank")
    chunks.append("")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Lab v6 for semantic-srcwalk potion-code-16M reranker")
    ...
    parser.add_argument("--no-embedding", action="store_true", help="Disable potion-code-16M reranker")
