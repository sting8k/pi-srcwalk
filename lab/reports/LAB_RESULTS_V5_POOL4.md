...
Total cases: 17
Summary: PASS=17, PARTIAL=0, FAIL=0, elapsed=66621ms
...
## Evidence expansion
### Expansion 1: context:framework/modelcatalog/pricing.go:28 (ok, 55ms)
```text
...
## Commands executed
- [ok, 96ms, matches=10] symbol-exact: srcwalk discover CalculateCost --as symbol --scope . --limit 10 --budget 3000
...
## Evidence expansion
### Expansion 1: context:framework/modelcatalog/pricing.go:28-45 (ok, 62ms)
```text
...
### Expansion 2: context:tests/governance/test_utils.go:64-73 (ok, 47ms)
```text
...
### Expansion 3: context:framework/streaming/audio.go:117-199 (ok, 39ms)
```text
...
- [ok, 17121ms] embedding-rerank: CodeRankEmbed rerank 'how does model pricing cost calculation work?' --pool 4
- [ok, 65ms] symbol-glob: srcwalk discover '*calculation*' --as symbol --scope framework --limit 12 --budget 2500

## Notes
- BM25 index for scope `framework` has 889 chunks; cold build 567ms, query 570ms.
- CodeRankEmbed reranked BM25 pool; load=11531ms warm=False encode=5588ms rss_after=1527.2MB.

## Best candidates
1. `framework/modelcatalog/pricing.go:1-80` — score=117.2, source=embedding-rerank, kind=chunk
2. `framework/modelcatalog/pricing.go:771-850` — score=112.9, source=embedding-rerank, kind=chunk
3. `framework/modelcatalog/pricing_test.go:1121-1200` — score=105.1, source=embedding-rerank, kind=chunk

## Evidence expansion
### Expansion 1: context:framework/modelcatalog/pricing.go:1-80 (ok, 32ms)
```text
...
### Expansion 2: context:framework/modelcatalog/pricing.go:771-850 (ok, 32ms)
```text
...
### Expansion 3: context:framework/modelcatalog/pricing_test.go:1121-1200 (ok, 44ms)
```text
...
## Commands executed
- [ok, 42ms, matches=10] symbol-exact: srcwalk discover CalculateCost --as symbol --scope framework --limit 10 --budget 3000
...
## Evidence expansion
### Expansion 1: context:framework/modelcatalog/pricing.go:28-45 (ok, 45ms)
```text
# Context Packet: framework/modelcatalog/pricing.go:28-45
confidence: structural syntax
caveat: source-evidence navigation only; no runtime proof
...
### Expansion 2: trace-callers:CalculateCost (ok, 26ms)
```text
...
## Commands executed
- [ok, 43ms, matches=10] symbol-exact: srcwalk discover CalculateCost --as symbol --scope framework --limit 10 --budget 3000
...
## Evidence expansion
### Expansion 1: context:framework/modelcatalog/pricing.go:28-45 (ok, 44ms)
```text
# Context Packet: framework/modelcatalog/pricing.go:28-45
confidence: structural syntax
caveat: source-evidence navigation only; no runtime proof
...
### Expansion 2: trace-callees:CalculateCost (ok, 46ms)
```text
...
## Commands executed
- [ok, 46ms] file-deps: srcwalk deps framework/modelcatalog/pricing.go --budget 3500

## Best candidates
1. `framework/modelcatalog/pricing.go:1` — score=90.0, source=file-deps, kind=file

## Evidence expansion
### Expansion 1: deps:framework/modelcatalog/pricing.go (ok, 54ms)
```text
...
## Commands executed
- [ok, 677ms] bm25-prf: bm25 search 'tests for semantic cache' --scope tests
- [ok, 2506ms] embedding-rerank: CodeRankEmbed rerank 'tests for semantic cache' --pool 4

## Notes
- BM25 index for scope `tests` has 1194 chunks; cold build 675ms, query 677ms.
- CodeRankEmbed reranked BM25 pool; load=0ms warm=True encode=2505ms rss_after=1619.5MB.

## Best candidates
1. `tests/integrations/python/tests/test_anthropic.py:1541-1620` — score=137.0, source=embedding-rerank, kind=chunk
2. `tests/integrations/python/tests/test_anthropic.py:1471-1550` — score=134.9, source=embedding-rerank, kind=chunk
3. `tests/integrations/python/tests/test_bedrock.py:1681-1760` — score=132.5, source=embedding-rerank, kind=chunk

## Evidence expansion
### Expansion 1: context:tests/integrations/python/tests/test_anthropic.py:1541-1620 (ok, 84ms)
```text
...
### Expansion 2: context:tests/integrations/python/tests/test_anthropic.py:1471-1550 (ok, 83ms)
```text
...
### Expansion 3: context:tests/integrations/python/tests/test_bedrock.py:1681-1760 (ok, 54ms)
```text
...
## Commands executed
- [ok, 10682ms] bm25-prf: bm25 search 'where is unicorn payment teleport implemented?' --scope .
- [ok, 1960ms] embedding-rerank: CodeRankEmbed rerank 'where is unicorn payment teleport implemented?' --pool 4
- [ok, 123ms, matches=10] symbol-exact: srcwalk discover implemented --as symbol --scope . --limit 10 --budget 3000

## Notes
- BM25 index for scope `.` has 10871 chunks; cold build 10671ms, query 10682ms.
- CodeRankEmbed reranked BM25 pool; load=0ms warm=True encode=1951ms rss_after=2141.5MB.

## Best candidates
1. `docs/contributing/adding-a-provider.mdx:771-850` — score=94.6, source=embedding-rerank, kind=chunk
2. `docs/contributing/adding-a-provider.mdx:911-990` — score=92.4, source=embedding-rerank, kind=chunk
3. `docs/contributing/adding-a-provider.mdx:701-780` — score=90.3, source=embedding-rerank, kind=chunk

## Evidence expansion
### Expansion 1: context:docs/contributing/adding-a-provider.mdx:771-850 (ok, 14ms)
```text
...
### Expansion 2: context:docs/contributing/adding-a-provider.mdx:911-990 (ok, 14ms)
```text
...
### Expansion 3: context:docs/contributing/adding-a-provider.mdx:701-780 (ok, 14ms)
```text
...
## Commands executed
- [ok, 17ms] file-show: srcwalk show src/Uno.Foundation/Rect.cs --budget 3500
- [ok, 67ms] file-discover: srcwalk discover src/Uno.Foundation/Rect.cs --as file --scope . --limit 8 --budget 2500
...
## Commands executed
- [ok, 2423ms, matches=10] symbol-exact: srcwalk discover Rect --as symbol --scope . --limit 10 --budget 3000
...
## Evidence expansion
### Expansion 1: context:src/Uno.UWP/Graphics/Interop/Direct2D/D2D1RoundedRect.cs:8-8 (ok, 34ms)
```text
...
### Expansion 2: context:src/Uno.Foundation/FoundationFeatureConfiguration.cs:15-31 (ok, 34ms)
```text
...
### Expansion 3: context:src/Uno.UI.RuntimeTests/UITests/_Engine/QueryResult.cs:22-22 (ok, 34ms)
```text
...
## Commands executed
- [ok, 18065ms] bm25-prf: bm25 search 'how does remote control server start?' --scope src
- [ok, 2646ms] embedding-rerank: CodeRankEmbed rerank 'how does remote control server start?' --pool 4
- [ok, 2009ms, matches=2] symbol-glob: srcwalk discover '*control*' --as symbol --scope src --limit 12 --budget 2500

## Notes
- BM25 index for scope `src` has 32805 chunks; cold build 18038ms, query 18065ms.
- CodeRankEmbed reranked BM25 pool; load=0ms warm=True encode=2645ms rss_after=2886.2MB.

## Best candidates
1. `src/Uno.UI.RemoteControl.DevServer.Tests/AppLaunch/AppLaunchIntegrationTests.cs:1-80` — score=105.6, source=embedding-rerank, kind=chunk
2. `src/Uno.UI.RemoteControl.DevServer.Tests/AppLaunch/AppLaunchIntegrationTests.cs:141-220` — score=103.4, source=embedding-rerank, kind=chunk
3. `src/Uno.UI.RemoteControl.DevServer.Tests/AppLaunch/RealAppLaunchIntegrationTests.cs:1-80` — score=101.3, source=embedding-rerank, kind=chunk

## Evidence expansion
### Expansion 1: context:src/Uno.UI.RemoteControl.DevServer.Tests/AppLaunch/AppLaunchIntegrationTests.cs:1-80 (ok, 49ms)
```text
...
### Expansion 2: context:src/Uno.UI.RemoteControl.DevServer.Tests/AppLaunch/AppLaunchIntegrationTests.cs:141-220 (ok, 48ms)
```text
...
### Expansion 3: context:src/Uno.UI.RemoteControl.DevServer.Tests/AppLaunch/RealAppLaunchIntegrationTests.cs:1-80 (ok, 45ms)
```text
...
## Evidence expansion
### Expansion 1: context:src/Uno.UI.RemoteControl.Host/Program.Command.cs:20-222 (ok, 210ms)
```text
...
### Expansion 2: trace-callers:StartCommandAsync (ok, 319ms)
```text
...
## Commands executed
- [ok, 49ms] overview: srcwalk overview --scope src/Uno.UI.RemoteControl.Host --symbols
...
- [ok, 2225ms] embedding-rerank: CodeRankEmbed rerank 'tests for InitializeComponent analyzer' --pool 4

## Notes
- BM25 index for scope `src` has 32805 chunks; cold build 18038ms, query 41ms.
- CodeRankEmbed reranked BM25 pool; load=0ms warm=True encode=2183ms rss_after=2892.6MB.
...
## Evidence expansion
### Expansion 1: context:src/Uno.Analyzers.Tests/UnoInitializeComponentAnalyzerTests.cs:141-220 (ok, 43ms)
```text
...
## Commands executed
- [ok, 311ms] bm25-prf: bm25 search 'how does discover rank results?' --scope src
- [ok, 2050ms] embedding-rerank: CodeRankEmbed rerank 'how does discover rank results?' --pool 4
- [ok, 52ms, matches=6] symbol-glob: srcwalk discover '*discover*' --as symbol --scope src --limit 12 --budget 2500

## Notes
- BM25 index for scope `src` has 613 chunks; cold build 310ms, query 311ms.
- CodeRankEmbed reranked BM25 pool; load=0ms warm=True encode=2049ms rss_after=3145.2MB.

## Best candidates
1. `src/commands/find.rs:141-220` `run_text_or_filtered_with_artifact` — score=100.9, source=embedding-rerank, kind=chunk
2. `src/commands/find.rs:421-500` — score=98.8, source=embedding-rerank, kind=chunk
3. `src/cli_run.rs:701-780` `run` — score=96.7, source=embedding-rerank, kind=chunk

## Evidence expansion
### Expansion 1: context:src/commands/find.rs:141-220 (ok, 52ms)
```text
...
### Expansion 2: context:src/commands/find.rs:421-500 (ok, 44ms)
```text
...
### Expansion 3: context:src/cli_run.rs:701-780 (ok, 56ms)
```text
...
## Commands executed
- [ok, 29ms] symbol-exact: srcwalk discover rank_matches --as symbol --scope src --limit 10 --budget 3000
- [ok, 28ms] symbol-lower: srcwalk discover rank_matches --as symbol --scope src --limit 10 --budget 2500
- [ok, 51ms] symbol-glob: srcwalk discover '*rank_matches*' --as symbol --scope src --limit 10 --budget 2500
- [ok, 37ms, matches=96] text-any: srcwalk discover rank_matches,rank_matche,rank_match,calls --match any --as text --scope src --limit 10 --budget 3000
...
### Expansion 2: trace-callers:rank_matches (ok, 22ms)
```text
